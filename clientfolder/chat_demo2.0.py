...
img_savepath = args['server']['asset']
css = """
.container {
...
def _launch_demo():
    ...
    def get_video(role, _chatbot, task_history):
        ...
        #         time.sleep(0.4)
        #         count += 1
        #         _chatbot[-1][-1] = '。' * ((count % 6) - 1)
        #         yield _chatbot
        # loadthrd = Thread(target=loading_render)
        # loadthrd.start()
        def rqt_video(url_image, data):
            requests.post(url_image, json=data)
...
""")
        ...
        audio_input.stop_recording(add_text_asr, [chatbot, task_history, audio_input], [chatbot, task_history, audio_input]).then(
            predict, [task_history], [], show_progress=True).then(
                fn=get_video, inputs=[role_dropdown, chatbot, task_history], outputs=[chatbot]
            )
        ...
        gr.Examples(examples=[
                    {"files": [os.path.join(args['gradio']['temp_dir'], "happy_example.png")], "text": "你觉得我现在心情怎么样？"},
                    {"files": [os.path.join(args['gradio']['temp_dir'], "sad_example.png")], "text": "你觉得我现在心情怎么样？"},
